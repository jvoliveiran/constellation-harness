---
# OpenCode command — filename `status.md` becomes `/status`.
# Delta vs plugins/constellation/commands/status.md:
#   • Claude Code namespaces it as /constellation:status; OpenCode has no plugin
#     namespace, so it is just /status (rename the file to /cs-status to avoid clashes).
#   • Body references to the `constellation:orchestrator` skill → `orchestrator`
#     (OpenCode skills are not namespaced).
#   • `disable-model-invocation` (Claude Code) has no OpenCode equivalent and is dropped.
...
3. If it exists, render the **Progress Banner** (defined in the `orchestrator` skill, from `.constellation/tracks.json`) followed by the vertical step table — one row per step of the track, `✓` done / `▶` current / `·` pending, with per-step detail from state:
...
This command never modifies state and never resumes the workflow — it only reports. To continue an interrupted workflow, use `/resume`.
